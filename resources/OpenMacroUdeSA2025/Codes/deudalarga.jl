...
struct DeudaLarga{T<:DefaultCost} <: Default
...
function switch_δ!(dd::DeudaLarga, δ)
    dd.pars[:κ] = dd.pars[:r]+δ
    dd.pars[:δ] = δ
    nothing
end
...
function DeudaLarga(; T = OG,
    ...
    pars = Dict(:β => β, :γ => γ, :r => r, :ψ => ψ, :χ => χ, :ρy => ρy, :σy => σy, :κ => κ, :δ => δ, :ℏ => ℏ)
...
function q_iter!(new_q, dd::DeudaLarga)
    ...
    r, κ, δ, ℏ, ψ = (dd.pars[key] for key in (:r, :κ, :δ, :ℏ, :ψ))

    # 1 es repago, 2 es default
    itp_q = make_itp(dd, dd.q, 1)
    itp_qD = make_itp(dd, dd.q, 2)
